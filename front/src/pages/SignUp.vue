    ...
    export default {
        ...
        methods: {
            SubmitForm() {
                ...
                const url = 'http://' + VITE_SERVER_ADRESS + ':' + VITE_SERVER_PORT + '/auth/signup'; // Build url
                const options = {
                    method: 'POST',
                    headers: {
...
<template>
    <div class="flex min-h-full items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="w-full max-w-md space-y-8">
            <div>
                <img class="mx-auto h-16 w-auto" src="../../img/logo/icon-groupomania.webp" alt="Workflow" />
                <h2 class="mt-6 text-center text-3xl font-bold tracking-tight text-gray-900">Create an account
                </h2>
            </div>
            <form class="mt-8 space-y-6" :class="this.hasInvalidCredentials ? 'hasErrors' : ''" action="#"
                method="POST">
                ...
                <div class="-space-y-px rounded-md shadow-sm">
                    <div>
                        ...
                        <input id="email-address" name="email" type="email" autocomplete="email" required="true"
                            v-model="email" @invalid="markFormInvalid"
                    ...
                    <div>
                        ...
                        <input id="password" name="password" type="password" autocomplete="current-password"
                            required="true" v-model="password" @invalid="markFormInvalid"
                            class="relative block w-full appearance-none rounded-none border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-primary-light focus:outline-none focus:ring-primary-light sm:text-sm"
                            placeholder="Password" />
                    </div>
                    <div>
                        <label for="confirmPassword" class="sr-only">Confirm password</label>
                        <input id="confirmPassword" name="confirmPassword" type="password" autocomplete="current-password" required="true"
                            v-model="confirmPassword" @invalid="markFormInvalid"
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-sm">
                        <p>Already have an account ? <router-link to="/login"
                                class="font-medium text-primary-default hover:text-primary-light">Login</router-link></p>
                    </div>
                </div>
                ...
                <div>
                    <button type="submit" @click.prevent="SubmitForm" :disabled="hasInvalidCredentials"
                        ...
                        <span class="absolute inset-y-0 left-0 flex items-center pl-3">
                            <LockClosedIcon class="h-5 w-5 text-primary-verydark group-hover:text-primary-default"
                                aria-hidden="true" />
                        </span>
                        Create account
                    </button>
